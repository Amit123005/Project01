...
const Die_Zone_TrendE4 = ({ dateRange, shift, area, isCustomLineColors = false, isDashboard = true, dieZoneFilter}) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://163.125.102.142:5000/api/ext4_dz_trend?start_date=${startDate}&end_date=${endDate}&shift=${shift}`);
        if (!response.ok) {
...
export default Die_Zone_TrendE4;
