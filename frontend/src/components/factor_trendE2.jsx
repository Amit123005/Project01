...
const Factor_TrendE2 = ({ dateRange, shift, area, isCustomLineColors = false, isDashboard = true }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://163.125.102.142:5000/api/ext2_trend?start_date=${startDate}&end_date=${endDate}&shift=${shift}`);
        if (!response.ok) {
  ...
  const transformData = (data) => {
    const factorData = { id: 'Factor', color: 'hsl(220, 70%, 50%)', data: [] };
    ...
    if (Array.isArray(data) && data.length > 0) {
      data.forEach(item => {
        ...
        if (!isNaN(factorValue)) {
          factorData.data.push({ x: date, y: factorValue });
        }
      });
    } else {
    ...
    return [factorData].every(series => series.data.length === 0)
