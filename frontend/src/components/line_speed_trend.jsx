...
const Line_Speed_Trend = ({ dateRange, shift, area, isCustomLineColors = false, isDashboard = true }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://163.125.102.142:5000/api/line_speed_trend?start_date=${startDate}&end_date=${endDate}&shift=${shift}`);
        if (!response.ok) {
  ...
  const transformData = (data) => {
    const transformedData = [
      { id: 'Line_Speed', color: 'hsl(110, 70%, 50%)', data: [] }
    ];
    ...
    if (Array.isArray(data) && data.length > 0) {
      data.forEach((item) => {
        ...
        transformedData[0].data.push({ x: date, y: parseFloat(item.Line_Speed) });
      });
    } else {
...
export default Line_Speed_Trend;
